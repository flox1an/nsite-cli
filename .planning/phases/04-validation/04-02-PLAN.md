...
If any check fails, fix the issue in the relevant SKILL.md file before marking this task done.
  ...
  <verify>
    <automated>PASS=true; for d in .agents/skills/nsyte-setup .agents/skills/nsyte-deploy .agents/skills/nsyte-config .agents/skills/nsyte-auth .agents/skills/nsyte-ci .agents/skills/nsyte-concepts; do /tmp/skills-ref-venv/bin/skills-ref validate "$d" 2>&1 | grep -q "Valid skill" || { echo "FAIL validate: $d"; PASS=false; }; done; for f in .agents/skills/*/SKILL.md; do lines=$(wc -l < "$f"); skill=$(dirname "$f" | xargs basename); [ "$lines" -lt 500 ] || { echo "FAIL line budget ($lines/500): $skill"; PASS=false; }; desc=$(grep "^description:" "$f"); echo "$desc" | grep -qiE "(deploy|publish|host|install|configure|automate|manage|set up|provides)" || { echo "FAIL action vocab: $skill"; PASS=false; }; echo "$desc" | grep -qiE "(nostr|decentralized|static site|blossom|NIP-46)" || { echo "FAIL context vocab: $skill"; PASS=false; }; echo "$desc" | grep -qE "^description: [A-Z][a-z]+(s|es|ed)\b" || { echo "WARN verb-start: $skill — verify description starts with third-person verb"; }; done; $PASS && echo "ALL PASS"</automated>
  </verify>
